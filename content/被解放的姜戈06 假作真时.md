...
在Django中，对用户身份的检验，主要是在`views.py`中进行。`views.py`是连接模型和视图的中间层。HTTP请求会转给`views.py`中的对应处理函数处理，并发回回复。

在`views.py`的某个处理函数准备HTTP回复的过程中，我们可以检验用户是否登陆。根据用户是否登陆，我们可以给出不同的回复。最原始的方式，是使用`if`式的选择结构： 
